...
const incentive_service_1 = require("./incentive.service");
/**
 * Time Window Check:
...
async function generateSaturdayWeeklyReportData(refDate = new Date()) {
    ...
    await (0, incentive_service_1.syncMissingWorkerIncentives)();
    ...
    const incentives = await prisma_1.default.workerRegistrationIncentive.findMany({
        include: {
            agent: {
                select: {
                    id: true,
    ...
    // ==========================================
    const agentIncentiveSummaryRows = [];
    ...
    for (const agent of agents) {
        ...
        const thisMonthAmount = thisMonthList.reduce((sum, inc) => sum + (inc.amount || incentive_service_1.INCENTIVE_RATE_PER_WORKER), 0);
        const totalAmount = agentIncentives.reduce((sum, inc) => sum + (inc.amount || incentive_service_1.INCENTIVE_RATE_PER_WORKER), 0);
        ...
        agentIncentiveSummaryRows.push({
            ...
            incentiveRate: incentive_service_1.INCENTIVE_RATE_PER_WORKER,
    ...
    // ==========================================
    const workerIncentiveDetailRows = incentives.map((inc, idx) => ({
        sno: idx + 1,
        ...
        rate: incentive_service_1.INCENTIVE_RATE_PER_WORKER,
        amountCreditedToAgent: inc.amount || incentive_service_1.INCENTIVE_RATE_PER_WORKER,
    ...
    return {
        ...
        summary: {
            ...
            ratePerWorker: incentive_service_1.INCENTIVE_RATE_PER_WORKER
        }
    };
}
/**
...
async function generateSaturdayWeeklyExcelBuffer(refDate = new Date()) {
    ...
    data.agentIncentiveSummaryRows.forEach((r, idx) => {
    ...
    data.workerIncentiveDetailRows.forEach((r, idx) => {
